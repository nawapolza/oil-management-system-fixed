...
router.put('/users/:id', requireAuth, requireOwner, asyncHandler(async (req, res) => {
  ...
  // Keep empty optional fields as intentional edits (for example clearing a phone number).
  if (!update.username) return jsonResponse(res, { success: false, message: 'กรอก username' }, 422);
  if (cleanString(req.body.password)) update.password_hash = await bcrypt.hash(cleanString(req.body.password), 10);
  const result = await req.db.collection('users').updateOne({ _id: oid }, { $set: update });
  if (!result.matchedCount) return jsonResponse(res, { success: false, message: 'ไม่พบผู้ใช้งานนี้' }, 404);
...
router.put('/vehicles/:id', requireAuth, requireOwner, asyncHandler(async (req, res) => {
  ...
  if ((req.user.role || '') === 'owner') update.user_id = cleanString(req.body.user_id) || null;
  if (!update.plate_no) return jsonResponse(res, { success: false, message: 'กรอกเลขทะเบียนรถ' }, 422);
  const result = await req.db.collection('vehicles').updateOne(filter, { $set: update });
  if (!result.matchedCount) return jsonResponse(res, { success: false, message: 'ไม่พบรถคันนี้ หรือถูกปิดใช้งานแล้ว' }, 404);
